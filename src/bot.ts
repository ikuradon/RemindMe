...
const upsertUserData = async (
  ...
  await kv.atomic().set(primaryKey, userData).commit();
};
...
(async (_) => {
  ...
  const COOL_TIME_DUR_SEC: number = parseInt(
    Deno.env.get("COOL_TIME_DUR_SEC") || "60",
    10
  );
  ...
  sub.on("event", async (event: NDKEvent) => {
    if (
      ...
      !commandRegex.test(event.content) ||
    ...
    if (tzCommandRegex.test(command)) {
      ...
      if (reminderDate > new Date()) {
        ...
        const reply = createReplyTemplate(event.rawEvent());
        reply.content = `I will remind at ${formatTZ(
          reminderDate,
          "yyyy-MM-dd HH:mm",
          timezone
        )} (${timezone})`;
  ...
  Deno.cron("Send reminder", "* * * * *", async () => {
    ...
    await Promise.all(
      reminderList
        .filter((record) => record.remind_at <= current)
        .map(async (record) => {
